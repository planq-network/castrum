...
class TestBrowserProfileOneDevice(MultipleSharedDeviceTestCase):
    ...
    def test_profile_invite_friends(self):
        ...
        if not self.home.element_by_text_part('Hey join me on Status: https://join.planq.network/u/%s' % chat_key).is_element_displayed(20):
    ...
    def test_profile_add_remove_contact_via_contacts_view(self):
        ...
        users = {
            'scanning_ens_with_stateofus_domain_deep_link': {
                'contact_code': 'https://join.planq.network/u/%s.stateofus.eth' % ens_user_message_sender['ens'],
