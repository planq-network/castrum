(ns status-im.cosmos.account.staking.validators-list
...
(defn build-validator-row [item  index] (let [{:keys [identity name tokens operator-address]} item
     ...
     ;container view
      ...
      ; row with right
                                            ...
                                            [list-item/right-side {:accessory      :text
                                                                   :accessory-text (str tokens)
                                                                   :chevron        true}]]]))

(views/defview list-validators [address]
  (views/letsubs [input [:keplr-store/validators]]
    [:<>
     [containers/dynamic-data-container input
      ^{:key "list-validators"}
                                ;for performance
      [list/flat-list {:data      (get-in input [:data])
                       :render-fn build-validator-row}]]]))
